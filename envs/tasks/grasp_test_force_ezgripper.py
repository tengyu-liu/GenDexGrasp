...
class IsaacGraspTestForce_ezgripper(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        # Puhao:
        # [translation[:3], 6d_rot[3:9]]
        # ['left_ezgripper_knuckle_palm_L1_1', 'left_ezgripper_knuckle_palm_L1_2',
        # 'left_ezgripper_knuckle_L1_L2_1', 'left_ezgripper_knuckle_L1_L2_2']
    ...
    def _set_init_pose(self):
        ...
        # todo: for debug | data Aug. 29
        self.closure_q = self.init_q.clone()
        self.closure_q[:, 7:9] += 0.1
        self.closure_q[:, 10:12] += 0.1
        self.closure_q[:, 12:14] += 0.1
        for i_iter in range(self.cfg['eval_policy']['init']['steps']):
    ...
    def _load_agent(self, env_ptr, env_id):
        ...
        if self.dexterous_loaded == False:
            ...
            dexterous_asset_file = "movable_hand_urdf/ezgripper/robots/movable_ezgripper.urdf"
        ...
        hand_idx = self.gym.find_actor_rigid_body_index(env_ptr, dexterous_actor, "bh_base_link", gymapi.DOMAIN_SIM)
    ...
    def q_transfer_o2s(self, opt_q):
        ...
        # todo: to finish 911
        sim_joint_angle[:, 0] = opt_joint_angle[:, 0]
        sim_joint_angle[:, 1] = opt_joint_angle[:, 2]
        sim_joint_angle[:, 2] = opt_joint_angle[:, 1]
        sim_joint_angle[:, 3] = opt_joint_angle[:, 3]
    ...
    def get_q_as_opt(self):
        ...
        # sim_joint_angle[:, 2] = opt_joint_angle[:, 1]
        # sim_joint_angle[:, 3] = opt_joint_angle[:, 3]
        opt_q_angle[:, 0] = sim_q_angle[:, 0]
        opt_q_angle[:, 1] = sim_q_angle[:, 2]
        opt_q_angle[:, 2] = sim_q_angle[:, 1]
        opt_q_angle[:, 3] = sim_q_angle[:, 3]
        ...
        with tqdm(total=used_len) as pbar:
            ...
            if self.object_name is None:
                ...
                obj_asset = self.gym.load_asset(
                    self.sim, 'data', object_urdf_path, object_asset_options)
...
if __name__ == "__main__":
    ...
    cfg['agent']['dof_props']['velocity'] = 0.01
    init_opt_q = torch.zeros(16, 9+4, device='cuda')
    init_opt_q[:, 0] = 0.1
    init_opt_q[:, 3:9] = torch.tensor([1., 0., 0., 0., 1., 0.], device='cuda')
    hand_model = get_handmodel('ezgripper', 1, 'cuda', 1.)
    ...
    env = IsaacGraspTestForce_ezgripper(cfg, sim_params, gymapi.SIM_PHYSX, "cuda", 0, False,
