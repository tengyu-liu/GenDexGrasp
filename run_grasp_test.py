...
if __name__ == '__main__':
    ...
    if platform.node() == 'puhao-Nvidia-3090Ti':
        sim_headless = True
    elif platform.node()[:6] == 'lambda':
        sim_headless = True
    else:
        raise NotImplementedError('un-recognized platform')
    # cfg["env"]["enableCameraSensors"] = enable_camera
    device = "cuda"
    ...
    # split_path_func = lambda x: (x.split('-')[1], int(x.split('-')[2].split('.')[0]))
    # tra_path_list.sort(key=lambda x: (x.split('-')[1], int(x.split('-')[2].split('.')[0])))

    # init model
    # isaac_model = IsaacGraspTest(cfg, sim_params, gymapi.SIM_PHYSX, "cuda", 0, headless=sim_headless,
    #                              init_opt_q=opt_q, object_name=object_name, num_particles=1, fix_object=False)
    isaac_model = None
    if args.mode == 'debug':
        pass
    elif args.mode == 'test':
